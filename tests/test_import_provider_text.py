...
class ImportProviderTextTests(unittest.TestCase):
    def run_import(self, content, suffix='.txt', source_type='google_local_jsonl_import'):
        ...
        with acc.open(encoding='utf-8-sig') as af, rej.open(encoding='utf-8-sig') as rf:
            accepted = list(csv.DictReader(af))
            rejected = list(csv.DictReader(rf))
        return td, accepted, rejected, json.loads(rep.read_text())
