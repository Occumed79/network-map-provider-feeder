        ...
        with self.source_file.open("r", encoding="utf-8-sig", newline="") as f:
            for row in csv.DictReader(f):
                ...
                if wanted and country.lower() not in wanted:
                    continue
                self.sources.append({"country": country, "url": url})
    ...
    def start_requests(self):
        for source in self.sources:
            yield scrapy.Request(
                source["url"],
                callback=self.parse,
                cb_kwargs={"country": source["country"], "source_url": source["url"], "depth": 0},
                dont_filter=True,
            )

    def parse(self, response, country, source_url, depth):
        ...
        if page_matches:
            yield {
                ...
                "title": title,
                "anchor_text": "",
                "match_reasons": "; ".join(page_matches[:8]),
            }
        ...
        for link in response.css("a"):
            ...
            yield scrapy.Request(
                next_url,
                callback=self.parse,
                cb_kwargs={"country": country, "source_url": source_url, "depth": depth + 1},
            )
